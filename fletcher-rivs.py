...
def fletcher_rivs(x0, eps, eps1, eps2, M):
    x = x0
    k = 0
    #d = -grad_f(x0)
    d = 0
    while True:
        ...
        if k >= M:
            ...
            print(f'k >= 1, т.е. k = {k}')
            beta = (np.linalg.norm(grad))**2/(np.linalg.norm(grad_f(x0)))**2
            d = -grad+beta*d


        def f(alpha):
            x_new = [x[0] + alpha * d[0], x[1] + alpha * d[1]]
    ...
    return x
x0 = [0.5, 1]
x = fletcher_rivs(x0, eps=1e-5, eps1=0.1, eps2=0.15, M=10)
print(x)
